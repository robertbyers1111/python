...
def home():
    ...
    if 'Contents' in response and response['Contents']:
        photos = [s3_client.generate_presigned_url(
            ...
            ) for content in sorted(response['Contents'], key=lambda object: object['LastModified'], reverse=False)]
    ...
    if form.validate_on_submit():
        ...
        if image_bytes:
            ...
            all_labels = ['%s (%0.1f)'%(label['Name'],label['Confidence']) for label in response['Labels'] if label['Confidence'] > 67]
