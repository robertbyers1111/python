...
def home():
    ...
    if form.validate_on_submit():
        ...
        if image_bytes:
            ...
            print('\nx in response[Labels], dict items separated..')
            for x in response['Labels']:
                print(type(x),'Name:',x['Name'],'   Confidence:',x['Confidence'])

            #EXTRA CREDIT
            confthresh = 90
            all_labels = [label['Name'] for label in response['Labels'] if label['Confidence'] > confthresh]
